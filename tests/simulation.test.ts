...
describe("simulation kernel",()=>{
 ...
 it("fulfills basic needs automatically by priority when supplies exist",()=>{const s=new Simulation(10);const king=s.state.citizens.c_king,queen=s.state.citizens.c_queen;king.hunger=80;queen.hunger=80;king.thirst=80;queen.thirst=80;king.needPriority=100;queen.needPriority=50;s.state.resources.food=1;s.state.resources.water=1;s.advance(1);expect(king.hunger).toBeLessThan(queen.hunger);expect(king.thirst).toBeLessThan(queen.thirst);expect(s.state.resources.food).toBe(0);expect(s.state.resources.water).toBe(0);});
 it("uses a branching civilization tree to unlock buildings",()=>{const s=new Simulation(11);expect(s.isTechnologyAvailable("stoneworking")).toBe(true);expect(s.isTechnologyAvailable("sheltercraft")).toBe(false);expect(()=>s.startBuildingForAvailableWorker("shelter","Shelter","",12)).toThrow();const project=s.startResearchForAvailableWorker("stoneworking");expect(project.researcherIds).toHaveLength(1);project.researchDone=project.researchRequired-1;s.advance(1);expect(s.isTechnologyComplete("stoneworking")).toBe(true);expect(s.isTechnologyAvailable("sheltercraft")).toBe(true);const shelterResearch=s.startResearchForAvailableWorker("sheltercraft");shelterResearch.researchDone=shelterResearch.researchRequired-1;s.advance(1);expect(s.isTechnologyComplete("sheltercraft")).toBe(true);const building=s.startBuildingForAvailableWorker("shelter","Shelter","",12);expect(building.workerIds).toHaveLength(1);expect(s.state.resources.wood).toBe(22);expect(s.state.resources.stone).toBe(16);});
  it("preserves specialist jobs and produces the assigned resource",()=>{const s=new Simulation(12);const king=s.state.citizens.c_king;const start=s.state.resources.food;s.assignJob(king,"Gatherer");s.advance(1);expect(king.job).toBe("Gatherer");expect(s.state.resources.food).toBeGreaterThan(start);expect(king.order?.label).toBe("Gathering food");});
  it("does not overwrite an explicit specialist job with autonomous worker duty",()=>{const s=new Simulation(13);const king=s.state.citizens.c_king;s.assignJob(king,"Lumberjack");s.advance(3);expect(king.job).toBe("Lumberjack");expect(king.order?.kind).toBe("gather");});
});
